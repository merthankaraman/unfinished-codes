...
void seri_port_cevap(){
  ...
  while(espserial.available()){
    ...
    //Serial.print("gelen "); Serial.println(mesaj);
    mesaj_hazir = true;
  }
  if(mesaj_hazir > 0){
    ...
    if(doc["MQTT"] == "istek"){
      ...
      doc["MQTT"] = true;
      doc["SP/V"] = solar_V; doc["SP/A"] = solar_A; doc["SP/W"] = solar_W;
      
      doc["RT/V"] = ruzgar_V; doc["RT/A"] = ruzgar_A; doc["RT/W"] = ruzgar_W;
      
      doc["BT/V"] = cikis_V; doc["BT/A"] = cikis_A; doc["BT/W"] = cikis_W;
      
      int pwm_durum;
      sarj_durumu ? pwm_durum = pwm :pwm_durum = 0;
      doc["DR/PWM"] = pwm_durum; doc["DR/D"] = sarj_durumu;
