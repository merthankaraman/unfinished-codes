  ...
  if(fren_kontrol() > 0){
    ...
    if(motor_calis){
      ...
      while(motor_calis){
        ...
        motor_hiz = map(gaz_seviyesi,gaz_min_seviyesi,gaz_maks_seviyesi,pwm_min_deger,pwm_maks_deger);
        PWM_ayarla(motor_hiz);
        if(!tork_modu){
          ilk_defa_tork_modu = true;
          if(micros() - eski_zaman >= (gaz_seviye_tersi*100)){
            adimlar();
            siradaki_adim += yon;
