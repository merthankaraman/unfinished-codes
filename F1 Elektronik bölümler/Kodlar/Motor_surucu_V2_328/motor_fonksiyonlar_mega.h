...
void AH_BL(){
  ...
  TCCR3A =  0;            //OC2A - D11 normal port. 
  TCCR1A =  0x83;         //OC1A - D9 compare match noninverting mode, downcounting ,PWM 10-bit
}
//D9 PWM and D3 HIGH
void AH_CL(){
...
void BH_CL(){
  PORTD = c_dusuk_pin;      //Set D3 to HIGH and the rest to LOW
  TCCR3A =  0;            //OC2A - D11 normal port. 
  TCCR1A =  0x23;         //OC1B - D10 compare match noninverting mode, downcounting ,PWM 8-bit 
}
//D10 PWM and D5 HIGH
void BH_AL(){  
...
void CH_AL(){ 
  ...
  TCCR3A =  0x83;         // OC2A - D11 compare match noninverting mode, downcounting ,PWM 8-bit  
}
//D11 PWM and D4 HIGH
void CH_BL(){  
...
void PWM_ayarla(int genlik){
  ...
  if (tork_modu){
    ...
    OCR3A = genlik;                   // Set pin 11 PWM duty cycle
  }
  else{
    OCR1A = tork_disi_guc;
    OCR1B = tork_disi_guc;
